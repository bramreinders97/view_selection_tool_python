...
class ConfigCostEstimator:
    ...
    by summing the cost = storage_cost = (creation_cost * fudge_factor) for each model to be materialized
    ...
    def _calc_execution_cost(self, model: str) -> float:
        """
        Return the creation cost of a node
    ...
    def _calc_cost_model_set(self, model_set: Tuple[str] | List[str]) -> Tuple[float, float]:
        """
        Calculate the creation cost and storage cots of all models in a set of models,
    ...
    def _calc_cost_intermediate_models(self, config: Tuple[str]) -> Tuple[float, float]:
        """Return the total (storage and creation) costs of all the intermediate models in the current configuration"""
        return self._calc_cost_model_set(
            model_set=config
        )

    def _calc_cost_destination_nodes(self):
        """Return the total (storage and creation) costs of all the destination models in the current configuration"""
        return self._calc_cost_model_set(
